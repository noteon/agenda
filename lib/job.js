...
Job.prototype.computeNextRunAt = function() {
  ...
  function computeFromRepeatAt() {
    ...
    var lastRun = moment(this.attrs.lastRunAt || new Date()).startOf('minute').valueOf();
    var nextDate = moment(date(repeatAt)).startOf('minute').valueOf();
    var oneMin=1000*60;

    if (Date.now() === nextDate) {
      this.attrs.nextRunAt = undefined;
      this.fail('failed to calculate repeatAt time due to invalid format');
    } else if ((nextDate === lastRun) || (nextDate === (lastRun+oneMin)) || (nextDate === (lastRun-oneMin))) {
